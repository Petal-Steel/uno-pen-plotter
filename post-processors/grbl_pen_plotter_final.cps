...
description = "Grbl Pen Plotter (M3/M5)";
vendor = "grbl";
vendorUrl = "https://github.com/gnea/grbl/wiki";
longDescription = "Custom Grbl post for pen plotting from Fusion cutting/jet toolpaths. Jet ON outputs M3 (pen down); jet OFF outputs M5 (pen up).";
...
properties = {
  ...
  penLiftDelayMs: {
    title      : "Pen lift delay (ms)",
    description: "Delay after M5 before XY motion so the pen servo has time to lift clear of the paper.",
    group      : "preferences",
    type       : "integer",
    value      : 200,
    scope      : "post"
...
// Pen plotter has no commanded Z axis. Suppress all normal Z motion output.
zOutput.disable();
...
function onSection() {
  ...
  // Pen-plotter customization: jet/plasma sections use onPower() for M3/M5.
  // Do not start the milling spindle before the initial rapid on a jet section.
...
// Pen power is handled by COMMAND_POWER_ON/OFF in onCommand().
...
function writeProgramEnd() {
  ...
  onCommand(COMMAND_STOP_SPINDLE); // M5: pen up (includes configured pen-lift dwell)

  // G53 changes the coordinate system, but xOutput/yOutput are modal and may
  // otherwise suppress X4 or Y4 if the same numeric value was output earlier
  // in work coordinates. Reset both so the machine-coordinate park command
  // always explicitly contains BOTH X4 and Y4.
  xOutput.reset();
  yOutput.reset();
  gMotionModal.reset(); // force explicit G1 even if the previous drawing move was already G1
  forceFeed();
  writeBlock(gFormat.format(53), gMotionModal.format(1), xOutput.format(4), yOutput.format(4), feedOutput.format(2000)); // park at machine X4 Y4 at 2000 mm/min
...
function writeRetract() {
  // Pen plotter: no physical/logical Z retract axis. Pen lift is M5.
  return;
